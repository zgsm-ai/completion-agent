...
func (h *CompletionHandler) pruneCompletionCode(completionText, prefix, suffix, lang string) string {
	...
	var err error
	if len(config.Config().Wrapper.Prune.Pruners) > 0 {
		chain, err = NewPostprocessorChainByNames(config.Config().Wrapper.Prune.Pruners)
		if err != nil {
			zap.L().Error("Invalid config: 'customPruners'",
				zap.Any("customPruners", config.Config().Wrapper.Prune.Pruners))
		}
	}
	if chain == nil {
