 ...
 * - 用于区分不同类型的处理逻辑
 * @example
 * var processorType PostprocessorType = TypeDiscarder
 ...
 * - 用于构建处理器链
 * @example
 * processor, exists := postprocessorDefs["discard-extreme_repetition"]
 * if exists {
 ...
 *
 * func (p *MyProcessor) Process(ctx *PostprocessorContext) bool {
 *     // 处理逻辑
 *     return modified
 * }
 *
 ...
 * }
 *
 * func (p *MyProcessor) Type() PostprocessorType {
 *     return TypeCutter
 * }
 */
...
/**
 * 创建新的后置处理器链
 * @param {[]Postprocessor} discarders - 丢弃类型处理器列表
 * @param {[]Postprocessor} cutters - 裁剪类型处理器列表
 * @returns {*PostprocessorChain} 返回初始化好的处理器链
 * @description
 * - 使用提供的丢弃器和裁剪器创建处理器链
 * - 初始化命中处理器列表为空
 * - 返回可执行的处理器链实例
 * - 用于自定义处理器组合
 * @example
 * discarders := []Postprocessor{&ExtremeRepetitionDiscarder{}}
 * cutters := []Postprocessor{&RepetitiveTextCutter{}}
 * chain := NewPostprocessorChain(discarders, cutters)
 */
 ...
 * @returns {*PostprocessorChain, error} 返回处理器链和错误信息
 * @description
 * - 根据处理器名称查找对应的处理器实例
 * - 将处理器按类型分组到丢弃器和裁剪器
 * - 如果遇到无效的处理器名称，返回错误
 ...
 * chain, err := NewPostprocessorChainByNames(names)
 * if err != nil {
...
/**
 * 创建默认的后置处理器链
 * @returns {*PostprocessorChain} 返回默认配置的处理器链
 * @description
 * - 创建包含标准处理器的默认链
...
*
* 处理丢弃类型的处理器
* @param {*PostprocessorContext} ctx - 后置处理器上下文
 ...
 *
 * func (p *MyDiscarder) Process(ctx *PostprocessorContext) bool {
 *     // 自定义丢弃逻辑
 ...
 *
 * func (p *MyCutter) Process(ctx *PostprocessorContext) bool {
 *     // 自定义裁剪逻辑
 *     return modified
 * }
 *
